...
typedef struct			// define a type to hold parameters 
{
  ...
  double b; 			// daming factor of oscillator
}
...
main ()
{
  ...
  double omega_ext = 1.;
  double phi_ext = 0.;
  double damping_force = 0.; 	//initial damping force
  double m = 1.;
  double k = pow (2. * pi, 2);
  double p = 2.;
  ...
  while (answer2 != 0)		// iterate until told to move on 
      ...
      while (answer != 0)	// iterate until told to move on 
	{
	  ...
	  cout << "[13] damping_force = " << damping_force << endl;
      ...
      else  // append
	{
      ...
      rhs_parameters.m = m;
      rhs_parameters.p = p;
      rhs_parameters.b = b; //setting b into structure
...
rhs (double t, double y[], int i, void *params_ptr)
{
  ...
  double b = ((force_parameters *) params_ptr)->b;  //added this for bamping
