...
//define a type to hold parameters
typedef struct
{
  double alpha;
  double beta;
}
ab_parameters;  //define a structure of this type
		// using the keyword ab_parameters
//************************** main program ***************************
int
main (void)
{
  ...
  //double alpha_double = 1.;		// a parameter for the function 
  ...
  //object for ab_parameters
  ab_parameters my_alphaBeta;
  //set up values for ab_parameters structure
  my_alphaBeta.alpha = 2.;
  my_alphaBeta.beta = 3./2.;
  
  ofstream out ("derivative_test.dat");	// open the output file 

  params_ptr = &my_alphaBeta;		// double to pass to function 
  ...
  out << "#Different derivative tests for numbers" << endl;
  out << "#Log(h)            Log(FD)          Log(CD)           Log(Extrap) " << endl;
...
test_function (double x, void *params_ptr)
{
  double passed_alpha;
  double passed_beta;
  //ab_parameters *passed_ptr;
  passed_alpha = ((ab_parameters *) params_ptr) ->alpha;
  passed_beta = ((ab_parameters *) params_ptr) ->beta;
  return (passed_alpha*pow(x, passed_beta));
}
...
test_function_derivative (double x, void *params_ptr)
{
  ...
  passed_alpha = ((ab_parameters *) params_ptr) ->alpha;
  passed_beta = ((ab_parameters *) params_ptr) ->beta;
  return (passed_alpha*passed_beta*pow(x,(passed_beta-1)));
}
