...
# titles and labels
set title 'Test of Numerical Derivatives using 2x^(3/2)'
...
f2(x) = a2*x + b2
fit [-2:-1] f2(x) "derivative_test.dat" using ($1):($3) via a2,b2 
...
plot "derivative_test.dat" using ($1):($2) title 'forward difference', \
    ...
    "derivative_test.dat" using ($1):($4) title 'extrapolated derivative', \
