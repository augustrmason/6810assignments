...
main (void)
{
  ...
  double hmin = 0.000001;		// starting h value 
  double hmax = 1.;		// last h value 
  ...
  out << scientific << setprecision (16)
    << hmin << "       "
    << y_euler[0] << "       "
    << y_rk4[0] << "       " << exact_answer (hmin, params_ptr) << endl;

 

  for (double h = hmin; h <= hmax; h += .001)
  {
    y_euler[0] = 1.0;		// initial condition for y(t) 
    y_rk4[0] = 1.0;		// initial condition for y(t)
    for (double t = tmin; t <= tmax; t += .01)
    {
     ...
     out << scientific << setprecision (16)
	      << h << "       "
	      << y_euler[0] << "       "
	      << y_rk4[0] << "       " << exact_answer (tmax + h, params_ptr) << endl;
